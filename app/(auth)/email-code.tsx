...
export default function EmailCodeScreen() {
  ...
  function onChange(v: string) {
    ...
    if (v.length === 6) {
      setTimeout(() => {
        if (v === DEMO_CODE) router.push("/setup-birth");
        else setError(true);
      }, 120);
    }
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#080A0B" }}>
      ...
      <View style={{ paddingHorizontal: 20, gap: 8, marginTop: 16 }}>
        <Text style={[typography.headline, { color: "white" }]}>Enter code</Text>
        <Text style={[typography.bodyLargeRegular, { color: "white" }]}>We sent it to {email || "your email"}</Text>
      </View>
